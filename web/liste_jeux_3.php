...
if($type_operation==2){
    $array_liste_collonne=array(
        array("nom","ASC","jeu_nom",null,"txt"),
        array("x","ASC","jeu_x","jeu_bgg_width","txt"),
        array("y","ASC","jeu_y","jeu_bgg_depth","txt"),
        array("z","ASC","jeu_z","jeu_bgg_length","txt"),
        array("poids","ASC","jeu_poids","jeu_bgg_weight","txt"),
        array("boîte","ASC","jeu_est_boite",null,"button"),
    );
...
while($bdd_liste_jeu = mysqli_fetch_object($res_liste_jeu)){
    ...
    if($type_operation==2){
        foreach ($array_liste_collonne as $col_actu){
            if(isset($col_actu[4]) and $col_actu[4]="button"){
                //$ligne_tableau .= '<button type="button" class="btn btn-primary">Primary</button>';
            }else{
                if(isset($col_actu[3])){
                    $calcul=null;
                    $calcul_txt=null;
