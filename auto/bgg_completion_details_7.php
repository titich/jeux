        ...
        while ($bdd_liste_jeux = mysqli_fetch_object($result_liste_jeux)) {
            if($array_detail_jeu_iter2=simplexml_load_file('https://www.boardgamegeek.com/xmlapi2/thing?id='.$bdd_liste_jeux->jeu_bgg_id.'&stats=1')){
                ...
                if($bdd_liste_jeux->jeu_bgg_description!=$description[0]){
                    $array_champ[]=array('jeu_bgg_description',$description[0]);
                    $array_champ[]=array('jeu_bgg_description_update_date',date("Y-m-d H:i:s"));
                }
                if($bdd_liste_jeux->jeu_bgg_nb_player_nb_votes!=$nbre_votes){
                    $array_champ[]=array('jeu_bgg_nb_player_nb_votes',$nbre_votes);
                    $array_champ[]=array('jeu_bgg_nb_player_best',$nb_player_best);
                    $array_champ[]=array('jeu_bgg_nb_player_recommended',$nb_player_recommended);
                }
