...
function bdd_ajout($ezine_db,$table,$array_champ){
    ...
    if(substr($table,-4)!="_jeu"){
        $sql_up_test = "SELECT `".$table."_nom_en`\n"
            . "FROM `".$table."` \n"
            . "WHERE `".$table."_bgg_id` = ".$array_champ[$table."_bgg_id"].";";
        //echo "<p>".$sql_up_test."</p>";
        $res_up_test = mysqli_query ($ezine_db, $sql_up_test) or ezine_mysql_die($ezine_db, $sql_up_test) ;
        $nbre_up_test=mysqli_num_rows($res_up_test);
        if($nbre_up_test!=0){
            $bdd_up_test = mysqli_fetch_object($res_up_test);
            if($bdd_up_test->{$table."_nom_en"} != $array_champ[$table."_nom_en"]){
                $sql_up = "UPDATE `".$table."` \n"
                    . "SET `".$table."_nom_en` = '".addslashes($array_champ[$table."_nom_en"])."' \n"
                    . "WHERE `".$table."_bgg_id` = ".$array_champ[$table."_bgg_id"].";";
                //echo "<p>".$sql_up."</p>";
        ...
        while ($bdd_liste_jeux = mysqli_fetch_object($result_liste_jeux)) {
            if($array_detail_jeu_iter2=simplexml_load_file('https://www.boardgamegeek.com/xmlapi2/thing?id='.$bdd_liste_jeux->jeu_bgg_id.'&stats=1')){
                ...
                if(count($poll)!=0){
                    ...
                    foreach ($poll as $poll_actu){
                        $poll_data_0=(array)$poll_actu;
                        //var_dump($poll_data_0);
                        if($poll_data_0["@attributes"]["name"]=="suggested_numplayers"){
                            $nb_player_nbre_votes=$poll_data_0["@attributes"]["totalvotes"];
                            if($bdd_liste_jeux->jeu_bgg_nb_player_nb_votes!=$nb_player_nbre_votes){
                                if($nb_player_nbre_votes>=1){
                        ...
                        }elseif($poll_data_0["@attributes"]["name"]=="suggested_playerage"){
                            //var_dump($poll_data_0);
                            $suggested_playerage=null;
                            $suggested_playerage_nbre_votes=$poll_data_0["@attributes"]["totalvotes"];
                            if($bdd_liste_jeux->jeu_bgg_suggested_playerage_nb_votes!=$suggested_playerage_nbre_votes){
                                if($suggested_playerage_nbre_votes>=1){
                                    $suggested_playerage_array=array();
                                    foreach ($poll_data_0["results"] as $poll_result_actu){
                                        $poll_result_actu=(array)$poll_result_actu;
                                        //var_dump($poll_result_actu["@attributes"]["numplayers"]);
                                        //var_dump($poll_result_actu);

                                        //var_dump($note_actu["@attributes"]["value"]);
                                        //var_dump($note_actu["@attributes"]["numvotes"]);
                                        $suggested_playerage_array[$poll_result_actu["@attributes"]["value"]]=$poll_result_actu["@attributes"]["numvotes"];

                                    }
                                    //var_dump($suggested_playerage_array);

                                    $suggested_playerage=array_search(max($suggested_playerage_array), $suggested_playerage_array);

                                    //var_dump($suggested_playerage);
                                    //echo "<p>".$nb_player_best."</p>";
                                    //echo "<p>".$nb_player_recommended."</p>";
                                    //var_dump($poll_data_0["results"]);
                                }
                ...
                if($bdd_liste_jeux->jeu_bgg_nb_player_nb_votes!=$nb_player_nbre_votes){
                    $array_champ[]=array('jeu_bgg_nb_player_nb_votes',$nb_player_nbre_votes,1);
                ...
                if($bdd_liste_jeux->jeu_bgg_suggested_playerage_nb_votes!=$suggested_playerage_nbre_votes){
                    $array_champ[]=array('jeu_bgg_suggested_playerage_nb_votes',$suggested_playerage_nbre_votes,1);
                    $array_champ[]=array('jeu_bgg_suggested_playerage',$suggested_playerage,0);
                }
                //var_dump($array_champ);
